...
# e.g. python 13_gst_transcode_only_split_stitch.py  -s ~/videos/bbb_sunflower_1080p_60fps_normal.mp4 -d ./output.mp4 -u 3 -i h264 -o h264 -b 5
...
def main():
    ...
    if split_count > num_dev2:
        print ("ERROR:Given -u split_count number is larger than avaiable number of devices , exiting....")
        raise SystemExit
    ...
    if split_count != 1:
        split_cmd = "gst-launch-1.0 filesrc location=" + \
                filename + \
                " ! qtdemux ! splitmuxsink max-size-time=" + str(split_length) + str("000000000") + " location=tmpfile" + "%02d." + filename[-3:] + " > stdout.log 2>&1 \n"
    else:
    ...
    for n in range(0, split_count):        
        ...
        if input_encoder == "h264" and output_encoder == "h264":
            transcode_cmd = "gst-launch-1.0 filesrc location=tmpfile" + \
                    ...
                    " ! qtdemux ! h264parse ! vvas_xvcudec  dev-idx="+str(n)+" ! queue ! vvas_xvcuenc dev-idx="+str(n)+" target-bitrate="+br+" max-bitrate="+br+ \
            ...
            transcode_cmd = "gst-launch-1.0 filesrc location=tmpfile" + \
                    format(n, '02d') + filename[-4:] + \
                    " ! qtdemux ! h265parse ! vvas_xvcudec  dev-idx="+str(n)+" ! queue ! vvas_xvcuenc dev-idx="+str(n)+" target-bitrate="+br+" max-bitrate="+br+ \
                    " periodicity-idr=120 "+"num-slices=4 num-cores=4 "+ \
                    "!  h265parse ! qtmux ! fpsdisplaysink video-sink=\"filesink location=tmpfileout" + \
                    format(n, '02d') + ofilename[-4:] + \
                    "\" text-overlay=false sync=false -v" + \
    ...
    for n in range(0, extra_chunks):
        ...
        if input_encoder == "h264" and output_encoder == "h264":
            transcode_cmd = "gst-launch-1.0 filesrc location=tmpfile" + \
                    format(split_count+n, '02d') + filename[-4:] + \
                    " ! qtdemux ! h264parse ! vvas_xvcudec  dev-idx="+str(n)+" ! queue ! vvas_xvcuenc dev-idx="+str(n)+" target-bitrate="+br+" max-bitrate="+br+ \
                    " periodicity-idr=120 "+"num-slices=4 num-cores=4 "+ \
                    "!  h264parse ! qtmux ! fpsdisplaysink video-sink=\"filesink location=tmpfileout" + \
