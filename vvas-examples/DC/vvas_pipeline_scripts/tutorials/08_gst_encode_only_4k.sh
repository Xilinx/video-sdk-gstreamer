...
#checking width and height from filename for rawfiles 
...
if [[ $width_val = 3840 || $height_val = 2160 ]]
then
    echo ""
else
    echo "Please provide input with width:3840 and height:2160 raw file."
    exit 1
fi
...
pipe="filesrc location=$2
	! queue
	! rawvideoparse format=$format_name width=3840 height=2160 framerate=60/1
	! vvas_xvcuenc dev-idx=$di target-bitrate=20000 max-bitrate=20000 num-slices=4
	! h264parse
	! qtmux
