...
if [[ ("$1" = "Edge") || ("$1" = "EDGE") || ("$1" = "edge") ]]; then
  ...
  TARGET="PCIE"
  LIB="lib"
  PREFIX=$PWD/install/opt/xilinx/vvas/
  ENABLE_XRM=true
  ENABLE_PPE=false
  if [[ ("$2" = 0) ]]; then
    ENABLE_XRM=false
...
if [[ "$TARGET" == "EDGE" ]]; then #TARGET == EDGE
  ...
  if [ $INSTALL_ACCEL_SW = true ]; then
    ...
    sed -E 's@<SYSROOT>@'"$SDKTARGETSYSROOT"'@g; s@<NATIVESYSROOT>@'"$OECORE_NATIVE_SYSROOT"'@g' meson.cross.template > meson.cross
    cp meson.cross $OECORE_NATIVE_SYSROOT/usr/share/meson/aarch64-xilinx-linux-meson.cross
    meson build --cross-file meson.cross --prefix /usr -Dvvas_xmongodblib=disabled
    cd build
    ninja
    DESTDIR=$SDKTARGETSYSROOT ninja install
    DESTDIR=../../install ninja install
  ...
  cd $BASEDIR/install
  tar -pczvf vvas_installer.tar.gz opt

  cd $BASEDIR

  echo -e "\n\n\tVVAS build is completed and stored in tar. Installng it..."
  echo -e "\tsudo tar -xvf install/vvas_installer.tar.gz -C /\n"
  sudo tar -xvf install/vvas_installer.tar.gz -C /
  echo -e "\n\tVVAS installation is completed.......\n"
