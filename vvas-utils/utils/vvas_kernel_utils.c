...
static int
alloc_xrt_buffer (xclDeviceHandle handle, VVASFrame * frame,
...
static void
free_xrt_buffer (xclDeviceHandle handle, VVASFrame * frame)
{
  if (!handle || !frame) {
  ...
  if (handle && frame->bo[0] > 0)
    xclFreeBO (handle, frame->bo[0]);
  memset (frame, 0x00, sizeof (VVASFrame));
}
...
vvas_free_buffer (VVASKernel * handle, VVASFrame * vvas_frame)
{
  if (vvas_frame->mem_type == VVAS_INTERNAL_MEMORY) {
    free_xrt_buffer (handle, vvas_frame);
  } else {
    if (!handle->free_func) {
      LOG_MESSAGE (LOG_LEVEL_ERROR,
...
vvas_sync_data (VVASKernel * handle, VVASSyncDataFlag flag, VVASFrame * frame)
{
  ...
  enum xclBOSyncDirection sync_flag =
      (flag ==
      VVAS_SYNC_DATA_TO_DEVICE) ? XCL_BO_SYNC_BO_TO_DEVICE :
