...
vvas_open_device (guint dev_idx)
{
...
pop_now:
  ...
  vvasmem->bo = xclAllocBO (priv->handle, size, XCL_BO_DEVICE_RAM, 0);
  ...
  if (params->flags & GST_VVAS_ALLOCATOR_FLAG_MEM_INIT) {
    ...
    data = xclMapBO(priv->handle, vvasmem->bo, true);
    if (data) {
      memset(data, 0, size);
      iret = xclSyncBO (priv->handle, vvasmem->bo, XCL_BO_SYNC_BO_TO_DEVICE, size, 0);
      if (iret != 0) {
        GST_ERROR_OBJECT (vvas_alloc, "failed to sync output buffer. reason : %d, %s",
            iret, strerror (errno));
        xclUnmapBO(priv->handle, vvasmem->bo, data);
        return NULL;
      }
      xclUnmapBO(priv->handle, vvasmem->bo, data);
    }
  ...
  if (priv->need_dma) {
    prime_fd = xclExportBO (priv->handle, vvasmem->bo);
    if (prime_fd < 0) {
      GST_ERROR_OBJECT (vvas_alloc, "failed to get dmafd...");
      xclFreeBO (priv->handle, vvasmem->bo);
      vvasmem->bo = 0;
      return NULL;
    }
...
gst_vvas_mem_map (GstMemory * mem, gsize maxsize, GstMapFlags flags)
{
  ...
  vvasmem->data =
      xclMapBO (alloc->priv->handle, vvasmem->bo, flags & GST_MAP_WRITE);
...
gst_vvas_mem_unmap (GstMemory * mem)
{
  ...
  if (vvasmem->data && !(--vvasmem->mmap_count)) {
    //ret = xclUnmapBO (alloc->priv->handle, vvasmem->bo, vvasmem->data);
...
gst_vvas_allocator_free (GstAllocator * allocator, GstMemory * mem)
{
  ...
  if (vvasmem->bo != NULLBO && mem->parent == NULL)
    xclFreeBO (alloc->priv->handle, vvasmem->bo);
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_DEVICE_INDEX:
      ...
      alloc->priv->handle = vvas_open_device (alloc->priv->dev_idx);
      break;
    case PROP_NEED_DMA:
...
gst_vvas_allocator_class_init (GstVvasAllocatorClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_DEVICE_INDEX,
      g_param_spec_uint ("device-index", "VVAS device index",
          "VVAS device index to allocate memory", 0, G_MAXUINT,
...
gst_vvas_allocator_get_paddr (GstMemory * mem)
{
  ...
  if (!xclGetBOProperties (alloc->priv->handle, vvasmem->bo, &p)) {
    return p.paddr;
  } else {
...
gst_vvas_memory_sync_with_flags (GstMemory * mem, GstMapFlags flags)
{
  ...
  if ((flags & GST_MAP_READ) && (vvasmem->sync_flags & VVAS_SYNC_FROM_DEVICE)) {
    ...
    iret =
        xclSyncBO (alloc->priv->handle, vvasmem->bo, XCL_BO_SYNC_BO_FROM_DEVICE,
        vvasmem->size, 0);
    if (iret != 0) {
  ...
  if (flags & GST_MAP_WRITE) {
    ...
    /* vvas plugins does XCL_BO_SYNC_BO_TO_DEVICE to update data, for others dont care */
...
gst_vvas_memory_sync_bo (GstMemory * mem)
{
  ...
  if (vvasmem->sync_flags & VVAS_SYNC_TO_DEVICE) {
    ...
    iret =
        xclSyncBO (alloc->priv->handle, vvasmem->bo, XCL_BO_SYNC_BO_TO_DEVICE,
        vvasmem->size, 0);
    if (iret != 0) {
