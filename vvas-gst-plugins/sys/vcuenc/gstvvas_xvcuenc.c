...
#define DIV_AND_ROUND_UP(n, d) (((n) + (d) - 1) / (d))
  ...
  if (GST_VIDEO_INFO_FORMAT (info) == GST_VIDEO_FORMAT_NV12_10LE32) {
    ...
    //align->padding_right = ALIGN (stride, enc->stride_align) - stride;
    align->padding_right = 0;
    stride = ALIGN (stride, enc->stride_align);
  } else
...
#define GST_VVAS_VIDEO_ENC_TARGET_BITRATE_DEFAULT                            64
...
#define GST_VVAS_VIDEO_ENC_RC_MODE_DEFAULT                                FALSE
...
static const gchar *
vvas_get_vcu_level_string (const gchar * level)
{
...
//  const gchar *format = "NV12_10LE32";
  ...
  Level = vvas_get_vcu_level_string (enc->level);
  if (!Level) {
    Level = "1";
    GST_WARNING_OBJECT (enc, "wrong level %s received using default %s",
        enc->level, Level);
  ...
  if (enc->codec_type == VVAS_CODEC_H264) {
    ...
    sprintf (params, "[INPUT]\n"
        ...
        enc->color_description, enc->transfer_characteristics, 
        enc->color_matrix, ScalingList, LoopFilter, ConstIntraPred, PrefetchBuffer,
        enc->num_cores);
  }
...
vvas_xvcuenc_calculate_load (GstVvasXVCUEnc *enc, gint *load)
{
  ...
  *load = atoi((char*)(strtok(param.output, " ")));
  ...
  GST_INFO_OBJECT (enc, "need %d%% device's load", (*load * 100) / XRM_MAX_CU_LOAD_GRANULARITY_1000000);
  return TRUE;
}
...
vvas_xvcuenc_allocate_resource (GstVvasXVCUEnc * enc, gint enc_load)
{
  ...
  GST_INFO_OBJECT (enc, "going to request %d%% load using xrm", (enc_load * 100) / XRM_MAX_CU_LOAD_GRANULARITY_1000000);
  ...
  if (getenv("XRM_RESERVE_ID") || enc->priv->reservation_id) { /* use reservation_id to allocate decoder */
     ...
     cu_list_prop.cuProps[0].requestLoad = XRM_PRECISION_1000000_BIT_MASK(enc_load);
    ...
    if (enc->dev_index >= xclProbe ()) {
      GST_ERROR_OBJECT (enc, "Cannot find device index %d", enc->dev_index);
      GST_ELEMENT_ERROR (enc, RESOURCE, NOT_FOUND,
          ("failed to find device index %d", enc->dev_index), NULL);
      return FALSE;
    }
...
vvas_xvcuenc_create_context (GstVvasXVCUEnc * enc)
{
  ...
  if (((unsigned int)enc->dev_index) >= xclProbe ()) {
    GST_ERROR_OBJECT (enc, "device index %d is not valid", enc->dev_index);
    GST_ELEMENT_ERROR (enc, RESOURCE, FAILED, (NULL),
        ("device index %d is not valid", enc->dev_index));
    return FALSE;
  }

  if (enc->sk_cur_idx < 0 || enc->sk_cur_idx < ENC_MIN_SK_CU_INDEX ||
        enc->sk_cur_idx > ENC_MAX_SK_CU_INDEX) {
    GST_ERROR_OBJECT (enc, "softkernel cu index %d is not valid. valid range %d -> %d",
        enc->sk_cur_idx == DEFAULT_SK_CURRENT_INDEX ? enc->sk_cur_idx :
	enc->sk_cur_idx - ENC_SK_CU_START_OFFSET,
	         ENC_MIN_SK_CU_INDEX, ENC_MAX_SK_CU_INDEX);
    GST_ELEMENT_ERROR (enc, RESOURCE, FAILED, (NULL),
...
#ifdef ENABLE_XRM_SUPPORT
  priv->xcl_handle = xclOpen (enc->dev_index, NULL, XCL_INFO);
  if (!priv->xcl_handle) {
    GST_ERROR_OBJECT (enc, "failed to open device index %u",
        enc->dev_index);
    return FALSE;
  }
  ...
  if (download_xclbin (enc->xclbin_path, enc->dev_index, NULL,
  ...
  if (priv->cu_idx < 0) {
    GST_ERROR_OBJECT (enc, "invalid cu index %d", priv->cu_idx);
    GST_ELEMENT_ERROR (enc, RESOURCE, FAILED, (NULL),
  ...
  if (xclOpenContext (priv->xcl_handle, priv->xclbinId, priv->cu_idx, true)) {
    GST_ERROR_OBJECT (enc, "failed to do xclOpenContext...");
    return FALSE;
  }
...
vvas_xvcuenc_allocate_internal_buffers (GstVvasXVCUEnc * enc)
{
  ...
  iret =
      alloc_xrt_buffer (priv->xcl_handle, WARN_BUFF_MAX_SIZE,
      XCL_BO_DEVICE_RAM, MEM_BANK, priv->warn_buff_obj);
  if (iret < 0) {
    GST_ERROR_OBJECT (enc,
        "failed to allocate warning buffer..");
    goto error;
  }
...
vvas_xvcuenc_free_internal_buffers (GstVvasXVCUEnc * enc)
{
  ...
  if (priv->warn_buff_obj) {
    free_xrt_buffer (priv->xcl_handle, priv->warn_buff_obj);
...
vvas_xvcuenc_preinit (GstVvasXVCUEnc * enc)
{
  ...
  payload_data[num_idx++] = ((uint64_t)(priv->warn_buff_obj->phy_addr) >> 32) & 0xFFFFFFFF;
  ...
  if (payload_buf->warn_buf_size && payload_buf->warn_buf_size < WARN_BUFF_MAX_SIZE) {
    memset(priv->warn_buff_obj->user_ptr, 0, WARN_BUFF_MAX_SIZE);

    iret =
        xclSyncBO (priv->xcl_handle, priv->warn_buff_obj->bo,
        XCL_BO_SYNC_BO_FROM_DEVICE, payload_buf->warn_buf_size, 0);
    if (iret != 0) {
      GST_ERROR_OBJECT (enc, "synbo failed - %d, reason : %s", iret,
          strerror (errno));
      GST_ELEMENT_ERROR (enc, RESOURCE, SYNC, NULL,
    ...
    GST_WARNING_OBJECT (enc, "device warning: %s", (char*) priv->warn_buff_obj->user_ptr);
  }
...
vvas_xvcuenc_allocate_internal_pool (GstVvasXVCUEnc * enc, GstCaps * caps)
{
  ...
  allocator = gst_vvas_allocator_new (enc->dev_index, ENABLE_DMABUF);
...
static gboolean
vvas_xvcuenc_validate_buffer_import (GstVvasXVCUEnc * enc, GstBuffer * inbuf, gboolean *use_inpool)
{
  ...
  if (gst_is_vvas_memory (in_mem) &&
    ...
    if (vmeta) {
      ...
      if (vmeta->stride[0] % enc->stride_align || align_elevation % enc->height_align) {
        *use_inpool = TRUE;
...
static gboolean
vvas_xvcuenc_send_frame (GstVvasXVCUEnc * enc, GstVideoCodecFrame * frame)
{
  GstVvasXVCUEncPrivate *priv = enc->priv;
  sk_payload_data *payload_buf;
  unsigned int payload_data[1024];
  ...
  bret = vvas_xvcuenc_validate_buffer_import (enc, frame->input_buffer, &use_inpool);
  if (!bret)
    goto error;

  if (GST_BUFFER_FLAG_IS_SET (frame->input_buffer, GST_BUFFER_FLAG_GAP)) {
    GST_DEBUG_OBJECT (enc, "duplicate buffer %" GST_PTR_FORMAT,
        frame->input_buffer);
  }
  ...
  if (!g_hash_table_contains (priv->in_idx_hash, in_paddr)) {
    ...
    GST_DEBUG_OBJECT (enc, "insert new index %d with physical address %p and buffer %p",
        priv->num_in_idx, in_paddr, inbuf);
    cur_in_idx = priv->num_in_idx++;
  } else {
    cur_in_idx =
        GPOINTER_TO_INT (g_hash_table_lookup (priv->in_idx_hash, in_paddr));
    GST_DEBUG_OBJECT (enc, "acquired index %d with physical address %p and buffer %p",
        cur_in_idx, in_paddr, inbuf);
  }
  ...
  if (!g_hash_table_contains (priv->qbuf_hash, GINT_TO_POINTER (cur_in_idx))) {
    ...
    bufpool->memcount = 1;
    g_hash_table_insert(priv->qbuf_hash, GINT_TO_POINTER (cur_in_idx), bufpool);
  ...
  /* Force Key frame */
  force_key_frame = GST_VIDEO_CODEC_FRAME_IS_FORCE_KEYFRAME (frame);
  if (force_key_frame) {
...
#ifdef HDR_DATA_SUPPORT
  hdr_meta = gst_buffer_get_vvas_hdr_meta (frame->input_buffer);
  if (hdr_meta) {
    memcpy(&payload_buf->hdr_data, &hdr_meta->hdr_metadata, sizeof(vcu_hdr_data));
  }
#endif
  ...
  lameta = gst_buffer_get_vvas_la_meta (frame->input_buffer);
  if (lameta) {
    ...
    if (lameta->gop_length != enc->gop_length) {
      GST_ERROR_OBJECT (enc, "gop length in LAMeta (%u) is not equal to "
          "encoder's gop length (%u)", lameta->gop_length,
          enc->gop_length);
      invalid_param = TRUE;
    }
  ...
  if (payload_buf->freed_ibuf_index != 0xBAD) {
    ...
    GST_LOG_OBJECT (enc, "decrement memcount from %d to %d corresponding to index %d",
      bufpool->memcount, bufpool->memcount - 1, payload_buf->freed_ibuf_index);
      bufpool->memcount--;
    if (!bufpool->memcount) {
...
error:
  if (in_mem)
    gst_memory_unref (in_mem);
  if (inbuf && (inbuf != frame->input_buffer))
    gst_buffer_unref (inbuf);
  return FALSE;
}
    ...
    GstVideoCodecState * state)
{
  ...
  if ((enc->gdr_mode == GDR_VERTICAL || enc->gdr_mode == GDR_HORIZONTAL) &&
      enc->gop_mode != LOW_DELAY_P) {
    GST_ELEMENT_ERROR (enc, LIBRARY, SETTINGS, NULL,
        ("gdr-mode can't be used when gop-mode is not LOW_DELAY_P"));
    return FALSE;
  }
    ...
    GstVideoCodecFrame * frame)
{
  ...
  bret = vvas_xvcuenc_send_frame (enc, frame);
  if (!bret)
    goto error;
...
gst_vvas_xvcuenc_class_init (GstVvasXVCUEncClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_DEVICE_INDEX,
      g_param_spec_int ("dev-idx", "Device index",
          "FPGA Device index", -1, 31, DEFAULT_DEVICE_INDEX,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
          GST_PARAM_MUTABLE_READY));
